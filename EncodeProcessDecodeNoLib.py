...
class EncodeProcessDecode(snt.Module):
    ...
    def __init__(self, steps, learn_features, n_layers, lat_size, edge_feat_cnt, node_feat_cnt, la_norm=True,
        ...
        self.steps = steps
        self._to_nodes = self._make_mlp(self.lat_size)
        self._to_edges = self._make_mlp(self.lat_size)
    ...
    def _update_edge_features(self, grp):
        ...
        return self._to_edges(tf.concat(features, axis=-1))

    def _update_node_features(self, grp, new_edges):
        """Aggregrates edge features, and applies node function."""
        num_nodes = grp.nodes.shape[0]
        features = [grp.nodes, tf.math.unsorted_segment_sum(new_edges, grp.receivers, num_nodes)]
        return self._to_nodes(tf.concat(features, axis=-1))

    def _pass_message(self, graph):
        # apply edge functions
        new_edges = self._update_edge_features(graph)
        # apply node function
        new_nodes = self._update_node_features(graph, new_edges)
